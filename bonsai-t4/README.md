...
4. **Launch** with `--chat-template-kwargs '{"reasoning_effort": "medium"}'`
   ...
   ladder: dual/single `-ngl 99` @ **262144** (f16 KV) → 262144 q8_0 KV → 128000 → 65536 → 32768 ctx → CPU fallback.
...
7.21 GB weights split across 2×16 GB; 262144 ctx needs ~16.4 GB f16 KV (or ~8.2 GB at q8_0) — ladder falls back if it doesn't fit.
